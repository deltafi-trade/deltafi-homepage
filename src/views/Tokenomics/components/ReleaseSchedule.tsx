...
const labels = [2022, 2023, 2024, 2025, 2026, 2027, 2028].flatMap(year => [
    "JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"
].map(month => ({ year, month }))).slice(2, 74)//generated label from 2022 MAR to 2028 FEB (total 6 * 12 invest)
        ...
        <LegendUl >
            {
                payload.map((entry, index) => (
                    <LegendLi key={`item-${index}`}><svg width="14" height="14" viewBox="0 0 32 32">
...
A5.333333333333333,5.333333333333333,0,1,1,10.666666666666666,16"></path></svg>
                        <Typography fontSize={10} marginLeft={1}>{entry.value}</Typography></LegendLi>
                ))
            }
        </LegendUl>
    );
            ...
            <Box sx={{ display: { sm: "block", xl: "none" } }}>
                <StyledCt width="100%" height={450}>
                    ...
                    >
                        ...
                        <Area type="monotone" stackId="1" name="Community & Ecosystem" dataKey="ce" stroke="#693EFF" fill="rgba(153, 69, 253, 0.2)" />
                    </AreaChart>
                </StyledCt>
            </Box>
